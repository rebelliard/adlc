...
const CURSOR_MCP_BUNDLE = 'plugins/adlc-cursor/bin/adlc-mcp-wrapper.bundle.mjs';
...
/**
 * The only production bundle allowed to contain ticket-store writer code is
 * accepted only when it is the exact no-write output of the official builder.
 * This keeps the source boundary intact while detecting stale or hand-edited
 * generated bytes.
 */
export function isVerifiedCursorMcpBundle(name, body, root = ROOT) {
  if (name !== CURSOR_MCP_BUNDLE) return false;
  try {
    return Buffer.from(body).equals(buildCursorMcp({ root, write: false }).bundle);
  } catch {
    return false;
  }
}
...
export function isDirectWriterBypass(name, body) {
  ...
  if (isVerifiedCursorMcpBundle(name, body)) return false;
...
test('the generated Cursor MCP bundle is exempted only when builder-verified', () => {
  const path = join(ROOT, CURSOR_MCP_BUNDLE);
  const bundle = readFileSync(path);
  assert.equal(isVerifiedCursorMcpBundle(CURSOR_MCP_BUNDLE, bundle), true);
  assert.equal(isDirectWriterBypass(CURSOR_MCP_BUNDLE, bundle), false);
  assert.equal(
    isVerifiedCursorMcpBundle(CURSOR_MCP_BUNDLE, Buffer.concat([bundle, Buffer.from('\n// tampered')])),
    false,
    'a byte drift must lose the generated-bundle exemption',
  );
  assert.equal(
    isDirectWriterBypass(
      CURSOR_MCP_BUNDLE,
      "import { writeFileSync } from 'node:fs'; writeFileSync('.adlc/tickets.json', '{}');",
    ),
    true,
    'hand-written bundle-shaped source must remain subject to the guard',
  );
  assert.equal(
    isDirectWriterBypass(
      'plugins/adlc-cursor/bin/other.bundle.mjs',
      "import { writeFileSync } from 'node:fs'; writeFileSync('.adlc/tickets.json', '{}');",
    ),
    true,
    'the exemption must not apply to other bundles',
  );
});
